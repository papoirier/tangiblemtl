...
/*	function to automatically display all images attached to a post
	by http://www.rlmseo.com/blog/get-images-attached-to-post/


function img_getter($postId) {
...
// Bubble sort image object array by menu_order TODO: Turn this into std "sort-by" function in functions.php
	...
	foreach ($arrKeys as $iNum) {
		// Get the full image url for the attachment
		...
		$sImgString = '<a href="' . $sImageUrl . '">' .
			'<img src="' . $sImageUrl . '" alt="Image" title="Image" />' .
		'</a>';
...
// home page images
function image_attachment($key) {
	global $post;
	$custom_field = get_post_meta($post->ID, $key, true);

	if($custom_field) { //if the user set a custom field
		echo '<img class="lead_image" src="'.$custom_field.'" alt=""/>';
	}
	else { //else, return
		return;
	}
...
/*	Get post category
	Copyright 2008 Dirk Jaster  (email: dirk@freshheit.de)
*/
